...
画像生成APIを使わない場合は `IMAGE_API_PROVIDER=mock` のままで問題ありません。OpenAI画像生成に接続する場合は `OPENAI_API_KEY` を設定し、`IMAGE_API_PROVIDER=openai` にします。APIキーはサーバー側Route Handlerでのみ参照され、フロントには出ません。
