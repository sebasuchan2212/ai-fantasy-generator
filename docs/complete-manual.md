...
クレジットはユーザーアカウントに紐づくため、購入前にログインまたは新規登録が必要です。ただし、購入ボタンを押したユーザーが迷わないよう、未ログイン時は自動で `/auth` へ送り、ログイン後に同じプランのCheckoutを再開する導線にしています。
...
メールアドレスとパスワードで登録またはログイン
  ↓
元のプランのStripe Checkoutを自動開始
  ↓
決済完了後にクレジット付与
```

Supabaseでメール確認をONにしている場合、新規登録直後に確認メールが必要になることがあります。離脱を減らしたい初期運用では、メール確認OFFで開始し、迷惑利用が増えたらONにする方法もあります。

### 6.8 支払い方法を簡単にする

このアプリはStripe Checkoutを使っています。コード側で支払い方法を固定せず、Stripe Dashboardで有効化した支払い方法をStripeが自動で表示します。

おすすめ:

- クレジットカード: 必ず有効化
- Apple Pay / Google Pay: Stripe Dashboardで有効化
- PayPay: 日本向けに有効化。審査や利用条件が表示された場合はStripe Dashboardの案内に従う
- コンビニ決済: 必要なら有効化。ただし支払い完了まで時間差がある

楽天ペイはStripe Checkoutの標準決済手段としては扱えません。どうしても楽天ペイを入れたい場合は、KOMOJUやAdyenなど、楽天ペイ対応の決済プロバイダーを別途追加する設計になります。
